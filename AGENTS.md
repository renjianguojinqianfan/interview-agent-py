...
- `/implement` 的顺序是：TDD -> `/code-review` -> `/neat-freak`（§7 上下文同步） -> commit -> 关闭 issue，不得颠倒
- **TDD vertical slice**：测试与实现交替推进，禁止先写完所有实现再写测试（水平切片）
- **提交前 review 闸门**：`/code-review` 必须是 commit 前最后一个代码逻辑改动步骤；之后只允许 `/neat-freak`（文档同步）和 commit，不得再改 `app/` 或 `tests/` 下的代码逻辑
- **GC loop（防复发）**：`/code-review` 若发现 HARD 违反，修复时必须同步新增一个结构测试或 lint 规则防止同类违规复发（落在 `tests/test_architecture.py` 或对应守卫位）；纯 SOFT finding 的响应式小修无需新增规则。该规则是架构违规复利积累的闸门
- 关键转折点（实现完成 / review 完成 / 准备提交）重新加载对应技能 SKILL.md 逐字对照，不凭记忆
- 长对话中上下文会稀释早期加载的技能内容，todo list 和重新加载是对冲手段
- 详细流程、复审阈值与转折点操作见 `docs/workflow.md`
