...
> **进度注记**：6.1 + 6.2 + 6.3 + 6.4 全部完成（#12 + #13）。6.1-6.4 详见 #12 注记。6.3（#13 面试日程）已完成：InterviewSchedule ORM + 迁移 008 + 7 接口 CRUD（/api/interview-schedule）+ 邀约文本解析（规则解析 feishu/tencent/zoom 优先 -> LLM 兜底，规则算法隔离到 domain/services/schedule_parser.py 纯函数）+ APScheduler 调度框架（infrastructure/scheduler/manager.py，SchedulerManager 封装 AsyncIOScheduler，#18 可复用注册更多任务）+ 定时任务（每小时 cancel_expired_schedules，PENDING 且过期 -> CANCELLED）。/parse 端点限流 5/s（IP 维度），其余 6 端点无限流。APScheduler 3.11（4.x 仍 alpha）。R5 阶段 review 已完成（commit eea58b0，修复 6 项 finding）：6.1 接口数 15 -> 16（补 `POST /voice/tts/test` TTS 连通性测试，对齐 spec "测试连通性" 验收），ASR/TTS api_key 解除双向耦合（独立存储），邀约解析规则优先经 review 确认为合理增强，`_parsed_data_to_request` 时间格式异常降级，schedule_parser 抽共享正则，llm_registry 抽 `_get_global_setting`，删死防御代码。G.3 时区策略同步为 naive（反映现实，全表 aware 迁移留 stage 8）。

| # | 任务 | 接口数 | 说明 |
|---|------|--------|------|
| 6.1 | LLM 供应商 | 16 | `LlmProvider` + `LlmGlobalSetting` ORM + CRUD + 测试连通性（Provider/ASR/TTS）+ 默认供应商 + ASR/TTS 配置 + 热更新(reload) |
...
- 数据库：存 naive datetime（`DateTime` 无 `timezone=True`），部署服务器时区须为 UTC，使 naive 值等价于 UTC。R5 review 发现全部 ORM 模型与迁移均为 naive，与本节早期"存 aware UTC"表述漂移；全表迁移至 `timezone=True`（aware UTC）留 stage 8 收尾统一处理（涉及 resume/interview/knowledge_base/rag_chat/llm_provider/voice_config/interview_schedule 等存量表 + `datetime.now()` 调用点）
- API 序列化：Pydantic 输出 ISO 8601（如 `2026-07-14T12:00:00`）
- 前端 `new Date()` 自动转为本地时区显示，与 Java 的 `LocalDateTime` 行为在前端显示上一致
- **注意**：Java 返回无时区后缀的本地时间字符串，Python 返回 naive ISO 字符串，前端 `new Date()` 按本地时区解析；部署须保证服务器 UTC 以避免 `cancel_expired_schedules` 等定时任务的 `interview_time < now` 比较偏移
