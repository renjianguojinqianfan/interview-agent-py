...
| # | 缺失 | 修复方案 | 工作量 |
|---|------|---------|--------|
| ✅ F1 | 架构方向无机械执行 | `tests/test_architecture.py`：①domain 层禁止导入 fastapi/sqlalchemy/app.infrastructure/app.application/app.api ②domain 层仅允许 import 标准库。用 `ast` 解析，无需新依赖。**已完成（1d255de）** | 1h |
