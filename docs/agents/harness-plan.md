...
| # | 缺失 | 修复方案 | 工作量 |
|---|------|---------|--------|
| F1 | 架构方向无机械执行 | `tests/test_architecture.py`：①domain 层禁止导入 fastapi/sqlalchemy/app.infrastructure/app.application/app.api ②domain 层仅允许 import 标准库。用 `ast` 解析，无需新依赖 | 1h |
| F2 | 无已拒绝方案清单 | 新建 `docs/rejected-ideas.md`，汇总 ADR + 迁移计划中已否决的方案+理由 | 30min |
| F3 | infrastructure->application 依赖无 ADR | 新建 `docs/adr/0010-infrastructure-consumer-application-dependency.md` | 15min |

### Feedback（传感器）

| # | 缺失 | 修复方案 | 工作量 |
|---|------|---------|--------|
| F4 | CI 缺 `ruff format --check` | ci.yml 加 `uv run ruff format --check .` 步骤 | 5min |
| F5 | 文档漂移无自动检测 | `tests/test_doc_drift.py`：扫描 `docs/**/*.md` 中的文件路径引用，断言路径存在 | 1h |
...
| # | 缺失 | 修复方案 | 工作量 |
|---|------|---------|--------|
| F12 | 无 garbage collection loop | AGENTS.md §9 补充：每次 HARD 违反必须新增结构测试/lint 防复发 | 15min |
| F13 | 无 harness 质量度量 | `docs/agents/harness-coverage.md`：跟踪规则数 / 机械执行数 / 覆盖率 | 30min |
| F14 | CI 不运行 `make verify` | ci.yml 改为 `make verify` 或确保步骤对齐 | 15min |
...
**实现要点**：
- 用 `re.findall(r'`([^`]+\.\w+)`', content)` 提取路径引用
- 过滤：只保留含 `/` 或已知扩展名（.py/.md/.yml/.toml/.sql）的字符串
- 用 `pathlib.Path` 检查路径是否存在
...
#### F3: ADR for infrastructure->application

**文件**：`docs/adr/0010-infrastructure-consumer-application-dependency.md`

**内容**：记录 consumer（infrastructure）import application service 的妥协理由：consumer 属宿主/驱动角色，调用应用服务是常见模式
