...
**不变量**：前端零改动；每模块后端补齐对应契约测试；`make verify` 双栈全绿；对齐后各页面浏览器冒烟 0 致命 console error。前端声明但无页面调用的死端点（面试 `/sessions/{id}/report`；知识库 `/query`、`/query/stream`、`/uncategorized`；RAG `/{id}/knowledge-bases`；简历 `/health`）不实现，聚焦页面可用。知识库 `GET /{id}` 原属死端点，知识库面试功能域落地后前端题库页开始调用，已于 #42 转活实现（ADR-0017）。
